...
<div class="row">
  <div class="col-12">
    <div class="page-title-box d-flex justify-content-between align-items-center">
      <h4 class="page-title">Edit Service: {{ $service->name }}</h4>
      ...
      <div class="card">
        ...
        <div class="card-body">
          ...
          <div class="mb-3">
            ...
            <input type="text" name="short_description" class="form-control" value="{{ old('short_description', $service->short_description) }}" maxlength="500">
      ...
      <div class="card">
        ...
        <div class="card-body">
          ...
          <div id="detailsContainer">
            @php
              $details = old('details', $service->details->map(fn($d) => [
                'title' => $d->title,
                'content' => $d->content,
                'type' => $d->type,
              ])->toArray());
          ...
          <div class="row g-3 mb-3">
            <div class="col-md-6">
              ...
              <input type="text"
                     ...
                     class="form-control"
                     value="{{ old('why_choose_title', $service->why_choose_title) }}"
            ...
            <div class="col-md-6">
              ...
              <select name="why_choose_theme" class="form-select">
                <option value="dark" {{ old('why_choose_theme', $service->why_choose_theme ?? 'dark') === 'dark' ? 'selected' : '' }}>Dark (default)</option>
                <option value="light" {{ old('why_choose_theme', $service->why_choose_theme) === 'light' ? 'selected' : '' }}>Light</option>
              </select>
            </div>
            <div class="col-12">
              ...
              <textarea name="why_choose_intro"
                        class="form-control"
                        rows="3"
                        placeholder="Short intro about the service benefits">{{ old('why_choose_intro', $service->why_choose_intro) }}</textarea>
            </div>
          </div>

          @php
            $whyChoose = old('why_choose_features', $service->why_choose_features ?? []);
          @endphp
              ...
              <div class="why-choose-row border rounded p-3 mb-3 position-relative">
                ...
                <div class="row g-3">
                  ...
                  <div class="col-md-6">
                    ...
                    <input type="hidden" name="why_choose_features[{{ $index }}][existing_image]" value="{{ $feature['image'] ?? '' }}">
                  </div>
                  <div class="col-12">
                  ...
                  <div class="col-12">
                    ...
                    <textarea name="why_choose_features[{{ $index }}][items]" class="form-control" rows="3">{{ is_array($feature['items'] ?? null) ? implode("\n", $feature['items']) : ($feature['items'] ?? '') }}</textarea>
                  </div>
                  <div class="col-12 {{ !empty($feature['image']) ? '' : 'd-none' }} why-choose-preview">
                    <img src="{{ !empty($feature['image']) ? asset($feature['image']) : '' }}" alt="Preview" class="img-thumbnail" style="max-width: 240px;">
                  </div>
                </div>
              </div>
            @endforeach
    ...
    <div class="col-lg-4">
      <div class="card">
        ...
        <div class="card-body">
          <div class="mb-3">
            ...
            @if($service->image)
              <div class="mb-2">
                <img src="{{ asset($service->image) }}" alt="{{ $service->name }}" class="rounded" style="max-width: 100%; max-height: 150px; object-fit: cover;">
              </div>
            @endif
          ...
          <div class="mb-3">
            ...
            <input type="text" name="icon" class="form-control" value="{{ old('icon', $service->icon) }}" placeholder="e.g. fi fi-rr-oil-can">
          </div>
      ...
      <div class="d-grid">
        <button type="submit" class="btn btn-primary btn-lg">
          <i class="fi fi-rr-check me-2"></i> Update Service
        </button>
      </div>
    </div>
  </div>
    ...
    <div class="why-choose-row border rounded p-3 mb-3 position-relative">
      ...
      <div class="row g-3">
        ...
        <div class="col-md-6">
          ...
          <input type="hidden" name="why_choose_features[${whyChooseIndex}][existing_image]" value="">
        </div>
        <div class="col-12">
          <label class="form-label">Paragraph</label>
