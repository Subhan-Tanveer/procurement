...
function initSidebarMenu() {
	...
	if (!currentPath) currentPath = 'index-2.html';

	// Check if tab is already active (set by Laravel/PHP)
	var hasActiveTab = jQuery('.app-tab-content .tab-pane.active.show').length > 0;

	if (!hasActiveTab) {
		// reset only if no active tab is set
	...
	// Only try to match and activate if no tab is already active
	if (!hasActiveTab) {
		...
		// If tab is already active, just make sure the current menu item is highlighted
		var $activeLink = jQuery('.app-navbar a.active').first();
		if ($activeLink.length) {
			$activeLink.parents('.menu-inner').show().prev('a').addClass('active open');
		}
	}
}
